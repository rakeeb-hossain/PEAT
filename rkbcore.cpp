...
vector<vector<int > > seizureDetection(string path, QString reportName, vector<int> properties)
{
    ...
    while(n + 30 <= numOfFrames)
    {
        ...
        {
            for(int k = 0; k < frame1.cols - 400; k += 15)
            {
                int diagInt = 0;
                wasSeizure = false;
                avgLum = 0.0;
                ...
                for (num = 0; num < peaks.size(); num++)
                {
                    ...
                    if ((peaks[num] - min)/(min) > 0.10 && (peaks[num] - otherMin)/(otherMin) > 0.10)
                    {
                        ...
                        {
                            if (index[num] + n - lumDiagFrames[lumDiagFrames.size() - 1][0] > 2)
                            {
                                diagInt++;
                ...
                if (lumDiagFrames.size() != 0)
                {
                    if (lumDiagFrames[lumDiagFrames.size() - 1][1] >= 3)
                    {
                ...
                if (seizureCount != 0)
                {
                    if(seizureBoundaries[seizureBoundaries.size() - 1][1] - seizureBoundaries[seizureBoundaries.size() - 1][0] < (lumDiagFrames[lumDiagFrames.size() - lumDiagFrames[lumDiagFrames.size() - 1][1]][0]) - seizureBoundaries[seizureBoundaries.size() - 1][0])
                    {
                        ...
                        //cout << "Seizure at frames" << seizureBoundaries[seizureBoundaries.size() - 1][0] << "to" << seizureBoundaries[seizureBoundaries.size() - 1][1] << endl;
                    }
                    n += index[num];
                    wasSeizure = true;
                }
                ...
                if(index.size() > 0)
                {
                    ...
                    if (lumDiagFrames.size() > 1)
                    {
                        qDebug() << "------------------------------";
                        qDebug() << lumDiagFrames[lumDiagFrames.size() - 1][0];
                        qDebug() << lumDiagFrames[lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]][0];
                        qDebug() << lumDiagFrames[lumDiagFrames.size() - 1][1];
                        qDebug() << lumDiagFrames[lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]][1];
                        qDebug() << "------------------------------";
                        if (lumDiagFrames[lumDiagFrames.size() - 1][0] == lumDiagFrames[lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]][0])
                        {
                            if (lumDiagFrames[lumDiagFrames.size() - 1][1] <= lumDiagFrames[lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]][1])
                            {
                                qDebug() << n;
                                qDebug() << lumDiagFrames.size();
                                if (lumDiagFrames.size() - lumDiagFrames[lumDiagFrames.size() - 1][1] == lumDiagFrames.size() - 1)
                                {
                                    lumDiagFrames.erase(lumDiagFrames.begin() + lumDiagFrames.size() - 1);
                                }
                                else {
                                    lumDiagFrames.erase(lumDiagFrames.begin() + lumDiagFrames.size() - lumDiagFrames[lumDiagFrames.size() - 1][1], lumDiagFrames.begin() + lumDiagFrames.size() - 1);
                                }
                                qDebug() << lumDiagFrames.size();
                            }
                            else
                            {
                                lumDiagFrames.erase(lumDiagFrames.begin() + lumDiagFrames.size() - lumDiagFrames[lumDiagFrames.size() - 1][1] - lumDiagFrames[lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]][1], lumDiagFrames.begin() + lumDiagFrames.size() - 1 - lumDiagFrames[lumDiagFrames.size() - 1][1]);
                            }
                        }
                    }
                }
...
vector<vector<int > > seizureDetection(string path, QString reportName, vector<int> properties)
{
    ...
    vector<vector<int > > seizureBoundaries(round(numOfFrames/30), vector<int>(2));
    ...
    while(n + 30 <= numOfFrames)
    {
        ...
        for(int j = 0; j < frame1.rows; j += 16)
        {
            ...
            for(int k = 0; k < frame1.cols; k = k + 16)
            {
                ...
                /////////////////////////////////////////////////////
                ///THIS SHOULD ALL REMAIN COMMENTED
        ...
        double portionOfScreen = 1.0*(seizureCount*16.0*16.0)/(properties[2]*properties[3]);
        // double portionOfScreen = 1.0*(seizureCount*32.0*32.0)/(341.0*256.0);

        cout << seizureCount << endl;
        if (portionOfScreen > 0.25)
        {
            ...
            if (arb == 0)
            {
                ...
                {
                    QTextStream stream( &file );
                    stream << "Seizure at frames: " << n + 1 << " to " << n + 30 << endl;
                }
                arb++;
            }
            else {
        ...
        if (aarb == 0)
        {
            if ( aFile.open(QIODevice::ReadWrite|QIODevice::Text) )
            {
                QTextStream stream( &aFile );
                stream << properties[0] << endl;
                stream << portionOfScreen << endl;
            }
            aarb++;
        }
        else {
            QTextStream stream( &aFile );
            if (portionOfScreen > 1.0)
            {
                portionOfScreen = 1.0;
            }
            stream << portionOfScreen << endl;
